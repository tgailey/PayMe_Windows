...
namespace SigningIn_DatabaseV2
{
    public partial class CreateSurveyControl : UserControl
    {
        ...
        private void ClearInfoButton_Click(object sender, EventArgs e)
        {
            ...
            MonthBoxBefore.SelectedIndex = -1;
            DayBoxBefore.SelectedIndex = -1;
            YearBoxBefore.SelectedIndex = -1;
        ...
        private void CreateAccountButton_Click(object sender, EventArgs e)
        {
            ...
            else if (!(MonthBoxBefore.SelectedIndex == -1 && DayBoxBefore.SelectedIndex == -1 && YearBoxBefore.SelectedIndex == -1) && !(!(MonthBoxBefore.SelectedIndex == -1) && !(DayBoxBefore.SelectedIndex == -1) && !(YearBoxBefore.SelectedIndex == -1)))
            {
            ...
            try
            {
                ...
                using (var connection = new SqlConnection(OpeningForm.cb.ConnectionString))
                {
                    ...
                    if (MonthBoxBefore.SelectedIndex == -1)
                    {
                        bDateParameter = "";
                    }
                    else
                    {
                        fullouterBasic = "FULL OUTER JOIN BasicInfo ON BasicInfo.UserName = Users.UserName\n";
                        //      where = "WHERE ";
                        string dob = YearBoxBefore.SelectedItem.ToString() + "-" + (MonthBoxBefore.SelectedIndex + 1).ToString() + "-" + DayBoxBefore.SelectedItem.ToString();
                        bDateParameter = "BasicInfo.DateOfBirth < '" + dob + "' AND ";
                    }
                    if (MonthBoxAfter.SelectedIndex == -1)
                    {
                        aDateParameter = "";
                    }
                    else
                    {
                        fullouterBasic = "FULL OUTER JOIN BasicInfo ON BasicInfo.UserName = Users.UserName\n";
                        //      where = "WHERE ";
                        string dob = YearBoxAfter.SelectedItem.ToString() + "-" + (MonthBoxAfter.SelectedIndex + 1).ToString() + "-" + DayBoxAfter.SelectedItem.ToString();
                        aDateParameter = "BasicInfo.DateOfBirth > '" + dob + "' AND ";
                    }
                    ...
                    else
                    {
                        ...
                        label8.Text = (mbtitype[0] == 'I').ToString();
                        if (mbtitype[0] == 'I')
                        {
                            operator1 = "> 50";
                        }
                        ...
                        mbtiParamter = "PersonalityInfo.MBTI_EI " + operator1 + " AND PersonalityInfo.MBTI_NS " + operator2
                            + " AND PersonalityInfo.MBTI_TF " + operator3 + " AND PersonalityInfo.MBTI_JP " + operator4 + " AND ";
                    }
                    ...
                    #endregion
                    string wstatement = firstNameParameter + bDateParameter + aDateParameter + genderParameter + sexParameter + mbtiParamter + oteParameter + cParameter + eParameter + aParameter + nParameter;
                    cmd.CommandText = string.Format(@"INSERT INTO Surveys 
                                            (UserName, SurveyName, SurveyDescription, SurveyLink, ConfirmationString, SampleSize, CurrentResults, NumberOfQuestions, AmountPaid, Controls, WStatement, SubmittedUsers)
                                               VALUES
                                                 ('{0}', '{1}', '{2}', '{3}', '{4}', '{5}', 0, '{6}', '{7}', '{8}', '{9}', '');", 
                                                 OpeningForm._username, SurveyNameTextBox.Text, SurveyDescriptionTextBox.Text, SurveyLinkTextBox.Text, 
                                                 SurveyConfirmationTextBox.Text, samplesize, numquestions, amountpaid, control, wstatement.Replace('\'', '|'));
                    ...
                    string totalCommand = string.Format(@"UPDATE Users SET  Users.AvailableSurveys = Users.AvailableSurveys + '{0}/' 
                                                            FROM Users
                                                            " + fullouterBasic + fullouterPersonality + "WHERE Researcher = 0 AND " + wstatement, surveyid);
            ...
            {
                DisplayLabel.Text = "Connection Failed. " + ex.Message + ex.Procedure;
            }
        }
